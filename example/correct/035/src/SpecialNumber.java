...
public class SpecialNumber {
	...
	public static boolean isSpecial(int num) {
		...
		while (true) {
			...
			int quotient = number / divider;//get the quotient

			if (isPrime(quotient)) { // if quotient is prime, this is the last factor.
			...
			number = quotient; //if quotient is not prime, decomposes it again.
		}

		for (int item : listOfFactors) {//whether exactly 3 unique factors
			if (item == 0) {
				flagOfResult = false;
			}
		}
